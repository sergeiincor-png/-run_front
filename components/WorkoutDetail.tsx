...
const WorkoutDetail: React.FC<WorkoutDetailProps> = ({ date, session, onBack }) => {
  ...
  const calculatePace = (dur: number, dist: number) => {
    if (!dur || !dist) return "--:--";
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto w-full animate-in fade-in duration-300">
      {/* ВОТ ЭТА КНОПКА ВОЗВРАЩАЕТ К КАЛЕНДАРЮ */}
      <button 
        onClick={onBack} 
        className="flex items-center gap-2 text-slate-500 hover:text-white mb-10 font-bold uppercase text-[10px] tracking-widest transition-colors group"
      >
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] p-8 shadow-2xl relative overflow-hidden">
          <div className="relative z-10 text-white">
            ...
            <div className="flex items-end gap-2">
              <span className="text-6xl font-black">{data.fact?.distance_km || "0.0"}</span>
              <span className="text-xl font-bold opacity-60 mb-2">КМ</span>
              {data.fact && (
          ...
          <Activity size={180} className="absolute -right-10 -bottom-10 opacity-10 rotate-12" />
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="font-bold text-sm text-white">
              ...
              <tr>
                ...
                <td colSpan={2} className="p-6 pt-8 text-xs text-slate-300 leading-relaxed font-medium">
                  {data.plan?.description || "Описание тренировки не найдено."}
                </td>
              </tr>
            </tbody>
          </table>
