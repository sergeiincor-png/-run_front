...
const Profile: React.FC<ProfileProps> = ({ session, onBack }) => {
  ...
  // Состояние полей (инпуты всегда работают со строками)
  ...
  async function updateProfile(e: React.FormEvent) {
    ...
    try {
      ...
      // ВАЖНО: Преобразование типов для исключения ошибки 400
      // Используем только те колонки, которые точно есть в вашей базе (согласно скриншоту)
      const updates = {
        id: user.id,
        ...
        // created_at оставляем системным, updated_at добавляем только если создали колонку в БД
      };
      ...
      setMessage({ type: 'success', text: 'Данные атлета обновлены!' });
            ...
            <form onSubmit={updateProfile} className="space-y-8">
              <section>
                <h3 className="text-lg font-black italic uppercase mb-6 flex items-center gap-3">
                  <span className="w-1 h-6 bg-blue-600"></span> Профиль
                </h3>
              ...
              <button 
                type="submit" 
                disabled={saving}
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-black uppercase py-4 rounded-2xl transition-all flex items-center justify-center gap-3"
              >
