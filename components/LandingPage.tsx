    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      <div className="glass-card w-full max-w-md p-10 rounded-[3rem] relative border border-white/10 bg-[#1c1c1f] shadow-2xl">
        ...
        <form onSubmit={handleAuth} className="space-y-5">
          ...
          <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-lg hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
...
const LandingPage: React.FC = () => {
  ...
  const heroImage = 'https://images.unsplash.com/photo-1552674605-db6ffd4facb5?auto=format&fit=crop&q=80&w=2000';
  ...
  if (selectedArticle) {
    return (
      <div className="pt-32 pb-20 px-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <p className="text-xl text-slate-300 leading-relaxed whitespace-pre-wrap text-white">{selectedArticle.fullText}</p>
      </div>
    );
  }
    ...
    <div className="bg-[#09090b] min-h-screen">
      <nav className="fixed top-0 left-0 right-0 z-[60] py-8 px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between text-white">
          ...
          <button onClick={() => setIsAuthModalOpen(true)} className="bg-white text-black px-6 py-2.5 rounded-full text-sm font-bold hover:bg-slate-200 transition-all">Начать бесплатно</button>
        </div>
      </nav>
      ...
      <section className="relative pt-48 pb-32 text-center">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="mt-24 relative mx-auto max-w-5xl rounded-[3rem] overflow-hidden p-3 bg-white/5 border border-white/10">
